...
if [ ! $? = 0 ] || [ ! "$(dpkg-query -W --showformat='${db:Status-Status}' "whiptail" 2>&1)" = installed ]; then
     echo -e 'Whiptail is not installed!!\nWhiptail is required to run this programe!! '
    ...
    if [[ ${a} = "Y" ||  ${a} = "y" ]]; then #if the input is equall to y or Y then
      sudo apt install whiptail #install whiptail
    else #else exit
      echo "wrong input"
    exit 0
fi
